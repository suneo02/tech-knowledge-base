...
export const useFeturedListStore = create<{
  feturedTree: any[]
  feturedList: any[]
  preSearchList: any[]
  selectedKeys: any[]
  feturedTreeNum: any
  headerInfo: any
  feturedInfoInit: (keword: string) => void
  feturedDetailInit: (id: string) => void
  getFeturedTreeInfo: () => void
  getFeturedList: (data: any, keyword: any, type?: string) => Promise<any[]>
  addFeturedList: (data: any, type?: string) => Promise<any[]>
  setSelectedKeys: (selectedKeys: any[]) => void
  getfeturedDetailHeaderInfo: (id: any) => Promise<any>
}>((set, get) => ({
  feturedTree: [], //树
  feturedList: [], //榜单名录列表
  preSearchList: [], //预搜索数据
  ...
  getCorpNamepresearch: async (keyword, isClear = false) => {
    if (!isClear && !keyword) {
      return set({
        preSearchList: [],
      })
