...
const CustomRadio: React.FC<CustomRadioProps> = ({
  ...
  return (
    <Radio.Group
      ...
      size="small"
      {...rest}
      data-uc-id="x9UdhGCLkG"
      data-uc-ct="radio"
    >
      {options
        ?.filter((res) => !res.disabled)
        ?.map((item) => (
          <Radio key={item.value} value={item.value} data-uc-id="GHTod7zf5K" data-uc-ct="radio" data-uc-x={item.value}>
            {item?.label}
          </Radio>
        ))}
      {showCustom && (
        <Radio key={'custom'} value="custom" data-uc-id="Yud08WfJjE" data-uc-ct="radio" data-uc-x={'custom'}>
          <RangePicker
            size="small"
