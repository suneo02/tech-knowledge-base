...
/**
 * 从当前 URL 中获取指定参数的值。
 * 默认情况下不区分参数名称的大小写，可通过 caseSensitive 参数控制。
 *
 * @example
 * // 假设当前 URL 是 'http://example.com?name=Alice&Age=30'
 * const name = getUrlSearchValue('name'); // 返回 'Alice'
 * const age = getUrlSearchValue('age'); // 返回 '30' (不区分大小写)
 * const ageExact = getUrlSearchValue('Age', { caseSensitive: true }); // 返回 '30' (区分大小写)
 * const ageLower = getUrlSearchValue('age', { caseSensitive: true }); // 返回 undefined (区分大小写)
 * const gender = getUrlSearchValue('gender'); // 返回 undefined
 *
 * @param param - 需要获取其值的参数名。
 * @param options - 可选配置项。
 * @param options.caseSensitive - 是否区分大小写，默认为 false（不区分大小写）。
 * @returns 如果找到参数，则返回其值；否则返回 undefined。如果传入的参数名为空或仅包含空白字符，也返回 undefined。
 */
export const getUrlSearchValue = (param: string, options: { caseSensitive?: boolean } = {}): string | undefined => {
  if (!param?.trim()) {
    return undefined
  }
