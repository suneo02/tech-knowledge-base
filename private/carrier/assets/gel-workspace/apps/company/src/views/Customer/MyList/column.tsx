...
const NameRowComp: FC<{ row: DownloadFileResponse }> = ({ row }) => {
  const { itemMsg, downLoadFunName } = useMemo(() => getDownloadCfg(row), [row])
  return (
    <>
...
export const getMyListColumn = (getMyList) => {
  ...
  const columns: ColumnProps<DownloadFileResponse>[] = [
    {
    ...
    },
    {
      ...
      width: '200px',
      render: (_data, row) => {
        const { downFileType, href, downloadFileName, openPdf } = getDownloadCfg(row)
        return (
          <>
            {row?.status === 0 || row?.status === 1 ? (
              ...
              <>
                {(row?.status === 2 || row?.status === 4) && downFileType == 'pdf' && (
                  <span>
                    {' '}
                    <a href={openPdf} target="__blank" data-uc-id="KLtIYIljFn" data-uc-ct="a">
                      {intl('257641', '查看')}
                    </a>{' '}
                    &nbsp; &nbsp;
                  </span>
