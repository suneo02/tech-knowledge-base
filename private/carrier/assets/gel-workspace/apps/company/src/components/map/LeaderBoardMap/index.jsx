            ...
            <Spin spinning={loading} tip={`${intl('132761', '加载中')}...`} style={{ width: '100%' }}>
              <div style={{ width: 600, height: 400 }}>
                {data?.length === '0' && (
                  <Result
                    status={'no-data'}
                    title={intl('132725', '暂无数据')}
                    data-uc-id="gANSyFQCpq"
                    data-uc-ct="result"
                  />
                )}
                {data?.length && (
                  <>
                    ...
                    >
                      {intl(393233, '集团企业数量区域Top10')}
                    </p>
                    <BarChart
                      local={window.en_access_config ? 'en' : 'cn'}
                      type="bar"
                      meta={{
                        name: intl('208504', '企业数量'),
                        unit: intl('138901', '家'),
                      }}
                      chartData={memoData}
                    />
