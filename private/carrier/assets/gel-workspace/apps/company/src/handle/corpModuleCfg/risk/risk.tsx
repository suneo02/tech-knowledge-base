...
import { corpDetailRiskMenu } from '@/components/company/layoutConfig/risk'
...
const riskMenuMap = new Map(corpDetailRiskMenu.children.map((item) => [item.showModule, item]))

// 根据 showModule 获取菜单配置
const getMenuConfig = createGetMenuConfig(riskMenuMap, 'Risk')

// 模块标题配置
const moduleTitle: CorpModuleTitleCfg = {
  title: corpDetailRiskMenu.title,
  moduleKey: 'risk', // 与左侧大菜单齐名
  noneData: intl('363853', '暂无司法风险'),
}

// 裁判文书
const getcourtdecision: CorpSubModuleCfg = {
  cmd: 'detail/company/get_judgement',
  extraParams: (param) => {
    param.__primaryKey = param.companycode
    return param
  },
  title: getMenuConfig('getcourtdecision').showName,
  hint:
    '<i><div><span>' +
    intl(
      '365953',
  ...
  modelNum: getMenuConfig('getcourtdecision').countKey,
  bordered: 'dotted',
  ...
  skipTransFieldsInKeyMode: ['entityName'],
  columns: [
    null,
    {
      render: (txt, row) => {
...
// 立案信息
const getfilinginfo: CorpSubModuleCfg = {
  ...
  title: getMenuConfig('getfilinginfo').showName,
  downDocType: 'download/createtempfile/get_legalEvents',
  moreLink: 'getfilinginfo',
  modelNum: getMenuConfig('getfilinginfo').countKey,
  ...
  skipTransFieldsInKeyMode: ['parties'],
  columns: [
    null,
    {
      render: (txt, row, _idx) => {
...
// 开庭公告
const getcourtopenannouncement: CorpSubModuleCfg = {
  ...
  title: getMenuConfig('getcourtopenannouncement').showName,
  downDocType: 'download/createtempfile/get_courtSession',
  modelNum: getMenuConfig('getcourtopenannouncement').countKey,
  ...
  skipTransFieldsInKeyMode: ['content'],
  columns: [
    null,
    {
      render: (txt, row) => {
...
// 送达公告
const showDeliveryAnnouncement: CorpSubModuleCfg = {
  ...
  title: getMenuConfig('showDeliveryAnnouncement').showName,
  downDocType: 'download/createtempfile/get_ReachingAnnouncement',
  moreLink: 'showDeliveryAnnouncement',
  modelNum: getMenuConfig('showDeliveryAnnouncement').countKey,
  ...
  skipTransFieldsInKeyMode: ['roles'],
  columns: [
    null,
    {
      render: (txt, row) => {
...
// 法院公告
const getcourtannouncement: CorpSubModuleCfg = {
  ...
  title: getMenuConfig('getcourtannouncement').showName,
  ...
  modelNum: getMenuConfig('getcourtannouncement').countKey,
...
// 被执行人
const getpersonenforced: CorpSubModuleCfg = {
  ...
  title: getMenuConfig('getpersonenforced').showName,
  downDocType: 'download/createtempfile/get_executed_person',
  modelNum: getMenuConfig('getpersonenforced').countKey,
...
// 失信被执行人
const getdishonesty: CorpSubModuleCfg = {
  ...
  title: getMenuConfig('getdishonesty').showName,
  downDocType: 'download/createtempfile/get_discredit',
  moreLink: 'getdishonesty',
  modelNum: getMenuConfig('getdishonesty').countKey,
...
// 终本案件
const getendcase: CorpSubModuleCfg = {
  // apiSource: 'risk',
  ...
  title: getMenuConfig('getendcase').showName,
  downDocType: 'download/createtempfile/get_finalCase',
  moreLink: 'getendcase',
  modelNum: getMenuConfig('getendcase').countKey,
...
// 限制高消费
const getcorpconsumption: CorpSubModuleCfg = {
  title: getMenuConfig('getcorpconsumption').showName,
  ...
  modelNum: getMenuConfig('getcorpconsumption').countKey,
...
// 股权冻结
const equityfreeze: CorpSubModuleCfg = {
  custom: 'MaskRedirect',
  title: getMenuConfig('equityfreeze').showName,
  modelNum: getMenuConfig('equityfreeze').countKey,
  maskRedirect: {
    url: (companyCode: string) => getRiskOutUrl(RiskOutModule.EQUITY_FREEZE, companyCode),
  },
}

// 司法拍卖
const getjudicialsale: CorpSubModuleCfg = {
  ...
  title: getMenuConfig('getjudicialsale').showName,
  hint: intl(
    '365159',
    '司法拍卖，即人民法院在民事案件强制执行程序中，按程序自行进行或委托拍卖公司公开处理债务人的财产，以清偿债权人债权。法院拍卖前应当通知当事人。'
  ),
  modelNum: getMenuConfig('getjudicialsale').countKey,
...
// 破产重整
const showBankruptcy: CorpSubModuleCfg = {
  ...
  title: getMenuConfig('showBankruptcy').showName,
  downDocType: 'bankruptcy',
  modelNum: getMenuConfig('showBankruptcy').countKey,
...
// 悬赏公告
const rewardnotice: CorpSubModuleCfg = {
  custom: 'MaskRedirect',
  title: getMenuConfig('rewardnotice').showName,
  modelNum: getMenuConfig('rewardnotice').countKey,
  maskRedirect: {
    url: (companyCode: string) => getRiskOutUrl(RiskOutModule.REWARD_NOTICE, companyCode),
  },
}

// 限制出境
const exitrestriction: CorpSubModuleCfg = {
  custom: 'MaskRedirect',
  title: getMenuConfig('exitrestriction').showName,
  modelNum: getMenuConfig('exitrestriction').countKey,
  maskRedirect: {
    url: (companyCode: string) => getRiskOutUrl(RiskOutModule.EXIT_RESTRICTION, companyCode),
  },
}

// 询价评估
const getevaluation: CorpSubModuleCfg = {
  title: getMenuConfig('getevaluation').showName,
  hint: intl('365160', '询价评估的意思是法院对被执行人的资产进行询价并且请专人来评估。'),
  moreLink: 'getevaluation',
  modelNum: getMenuConfig('getevaluation').countKey,
  rightLink: (data) => {
    return (
...
// 导出司法风险模块配置（按 menu 配置顺序组织）
export const risk: CorpPrimaryModuleCfg = {
  moduleTitle,
  // 根据 menu 配置的顺序动态组织子模块
  ...Object.fromEntries(
    corpDetailRiskMenu.children.map((menuItem) => [menuItem.showModule, subModuleConfigs[menuItem.showModule]])
  ),
}
