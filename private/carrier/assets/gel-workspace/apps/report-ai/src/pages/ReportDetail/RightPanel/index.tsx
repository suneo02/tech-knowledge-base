import { RPReferenceView } from '@/components/Reference';
import { useMultiChapterGeneration, useReportFiles } from '@/store/reportContentStore/hooks';
import {
...
import { RefPreviewData } from '@/types';
...
export const RPRightPanel: FC = () => {
  ...
  const referenceMap = useRPDetailSelector(selectReferenceMap);

  // 获取带状态的引用列表（从 reportFiles 获取最新状态）
  const refList = useRPDetailSelector(selectSortedReferences);

  // 获取全局引用序号映射
  const ordinalMap = useRPDetailSelector(selectReferenceOrdinalMap);

  // 报告文件管理 Hook - 自动获取文件并同步到 Redux
  const { refreshFiles } = useReportFiles();

  // 从 Redux 读取报告级文件与置顶文件列表（派生）
  const topFiles = useRPDetailSelector(selectTopReportFiles);
  ...
  const handleFileStatusUpdate = useCallback(
    (statuses: FileStatusUpdate[]) => {
      dispatch(rpDetailActions.batchUpdateFileStatus(statuses));
    },
    [dispatch]
  );
