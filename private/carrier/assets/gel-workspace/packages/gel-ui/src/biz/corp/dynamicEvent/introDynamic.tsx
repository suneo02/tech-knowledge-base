...
export const getDynamicEventInnerContent = (
  ...
  try {
    const abstract = eachList?.event_abstract || {}
    const describe = eachList?.event_describe || {}
    const source_id = eachList?.event_source_id
    const companycode = eachList?.corp_id
    const event_id = eachList?.event_id
    ...
    if (type == '工商变更') {
      ...
      return (
        ...
        >
          <span className="dynamic-event-abstract underline-n">
            成为 "{formatText(abstract.projectname)}" 拟定供应商
          </span>
        </a>
      )
      ...
      return (
        <a
          className="w-link wi-link-color"
          target="_blank"
          href={`index.html#/patentDetail?type=patent&detailid=${formatText(event_id)}`}
          rel="noreferrer"
        >
          <span className="dynamic-event-abstract underline-n">
