...
export type ConvertedCascadeOption = {
  label: string
  value: string
  children?: ConvertedCascadeOption[]
  [key: string]: any
}
...
export const convertCascadeOptions = <OptionType extends Record<string, any>>(
  ...
  // 递归转换选项及其子选项
  const convertOptionsRecursively = (items: OptionType[]): ConvertedCascadeOption[] => {
    ...
    return items.map((opt) => {
      ...
      // 创建结果对象
      const result: ConvertedCascadeOption = { ...opt, label: '', value: '' }
