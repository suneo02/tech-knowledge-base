...
import { useMultiTableRef } from '../context'
import { TaskHistoryLog, TaskIdentifier, TaskStatusItem, useTableAITask } from '../context/TableAITaskContext'
import { useAITaskOperation } from '../hooks'
...
export const AITaskStatusPanel: React.FC = () => {
  ...
  // 使用AI任务操作Hook获取解析功能
  const { parseTaskId } = useAITaskOperation({ multiTableRef })

  // 获取任务信息
  const getTaskInfo = (taskId: string) => {
    if (!multiTableRef.current) return { columnName: '未知列', value: '未知值', columnId: '', rowId: '' }
    ...
    try {
      ...
      const value =
        rowIndex >= 0 && columnIndex >= 0 ? multiTableRef.current.getCellValue(columnIndex, rowIndex + 1) : '未知值'
