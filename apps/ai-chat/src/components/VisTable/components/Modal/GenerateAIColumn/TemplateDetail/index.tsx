...
import { Alert, Button, Card, Col, Divider, Row, Select, Spin, message } from '@wind/wind-ui'
...
const STRINGS = {
  BACK_BUTTON: t('464221', '返回'),
  USE_TEMPLATE_BUTTON: t('464100', '使用此模板'),
  TEMPLATE_NOT_FOUND: '未找到指定的模板或必要信息缺失。',
  INPUT_FIELDS_TITLE: t('464172', '自定义参数'),
  MODEL_INFO_TITLE: t('464182', '模型信息'),
  TOOLS_INFO_TITLE: t('464113', '工具信息'),
  PROMPT_TITLE: t('464143', '提示词'),
  FIELD_LABEL: t('205470', '标签'),
  FIELD_VALUE: t('464116', '值'),
  FIELD_DESCRIPTION: t('10647', '描述'),
  AI_MODEL_LABEL: t('464095', 'AI 模型'),
  LINK_TOOL_LABEL: t('464171', '联网工具'),
  ENABLED: t('464125', '已启用'),
  DISABLED: t('464162', '未启用'),
  NO_INPUT_FIELDS: t('464184', '此模板没有可自定义的参数。'),
  NO_MODEL_INFO: '未指定模型信息。',
  NO_TOOLS_INFO: '未配置工具。',
  LIMITED_FREE_TAG: t('283654', '限时免费'),
  NO_PROMPT_PROVIDED: '未提供提示词。',
}
    ...
    if (template && template.inputFields) {
      ...
      template.inputFields.forEach((field) => {
        ...
        if (field.type === 'multiSelect') {
          const options = field.options || columns
          initialFormValues[fieldName] = options.map((option) => String(option.value))
        } else if (field.type === 'select') {
          const options = field.options || columns
          const matchingOption = options.find((option) => option.label === field.title)
          initialFormValues[fieldName] = matchingOption ? String(matchingOption.value) : field.value
        } else if (field.type === 'textarea') {
          initialFormValues[fieldName] = field.value || field.recommend
        } else {
          initialFormValues[fieldName] = field.value
        }
      })
      ...
      {!template ? (
        ...
        <div className={styles[`${PREFIX}-container`]}>
          <Row className={styles[`${PREFIX}-content-row`]}>
            ...
            <Col span={16} className={styles[`${PREFIX}-right-panel`]}>
              ...
              <div className={styles[`${PREFIX}-prompt-display-area`]}>
                {template && template.prompt ? dynamicPrompt : STRINGS.NO_PROMPT_PROVIDED || '此模板没有提供提示词。'}
              </div>
            </Col>
          </Row>
          ...
          <div className={styles[`${PREFIX}-footer`]}>
            <Button onClick={handleGoBack}>{STRINGS.BACK_BUTTON}</Button>
            <Button type="primary" onClick={handleUseTemplate} style={{ height: 28, marginInlineStart: 8 }}>
