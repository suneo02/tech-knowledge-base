 ...
 * @returns 完整的超级聊天功能接口
 */
export const useChatSuper = (
  defaultMessages?: XChatConfig<SPAgentMsg, SPMsgParsed>['defaultMessages'],
  maxLength?: number
) => {
  ...
  const agentReqFunc = createAgentRequestHandler({
    ...
    sendAndInitializeConversation: ({ chatId, message, options, signal, onReciveQuestion }) => {
      const optionsParsed: ChatSenderOptions = options
        ? { ...options, clientType: 'superlist' }
   ...
   * 这提供了消息处理、状态管理和请求触发
   */
  const { onRequest, parsedMessages, setMessages } = useXChat<SPAgentMsg, SPMsgParsed>({
    // @ts-expect-error
    agent,
    parser: (message) => {
  ...
  const sendMessage = useCallback(
    (
      ...
      console.log('🚀 ~ sendMessage ~ deepSearch:', {
        role: 'user',
        content: message,
        agentId,
