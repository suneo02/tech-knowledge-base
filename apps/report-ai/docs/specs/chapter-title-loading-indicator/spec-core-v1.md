...
- 以 `useReportContentSelector(selectChapterAIMessageStatusMap)` 为核心数据源。Map 中状态为 `pending/receiving` 时即认为章节需要 Loading。@see apps/report-ai/src/store/reportContentStore/selectors/composition.ts:74
