...
import { aggregateFileData, isReportFileStatusMutable, mapToUnifiedList } from '@/domain/file';
...
import { selectChapters, selectReportContent, selectReportFiles } from './base';
 ...
 */
export const selectCanonicalChapterMap = createSelector(selectReportContent, (state) => {
 ...
 */
export const selectLeafChapters = createSelector([selectChapters], (chapters): RPLeafChapterEnriched[] => {
 ...
 */
export const selectLeafChapterMap = createSelector(
  [selectLeafChapters],
  (leafChapters): Map<string, RPLeafChapterEnriched> => {
...
/**
 * 选择统一文件列表（保持 Map 的插入顺序）
 */
export const selectFileUnifiedList = createSelector([selectFileUnifiedMap], (map) => mapToUnifiedList(map));
