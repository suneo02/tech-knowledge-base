...
import { getCreditRPAppendix, getCreditRPComment, RP_PRINT_CONSTANTS } from 'report-util/constants'
import { getCorpName } from 'report-util/misc'
import { TableSectionsPrinter } from '../../comp/TableSectionsPrinter'
 ...
 */
export class CreditRPPrintRenderer {
  /**
  ...
  private sectionRenderer: TableSectionsPrinter | null = null
  ...
  public initialize(): void {
    try {
      ...
      this.sectionRenderer = new TableSectionsPrinter(rpPrintStore.getData()?.reportConfig, {
        // 处理表格数据加载完成
  ...
  private renderComment(): void {
    ...
    const $coverJQuery = createRPComment(getCreditRPComment(rpPrintStore.getData()?.reportConfig, t, isEnForRPPrint()))
