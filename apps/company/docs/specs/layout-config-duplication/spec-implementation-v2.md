...
- `src/hooks/useModuleConfigLoader.ts` - 配置加载 Hook
- 配置缓存机制
- 单元测试
...
- @see [方案设计](./spec-design-v1.md#配置加载管理器)
...
- `src/config/unified-module-config.ts` - 统一配置源（轻量级元数据）
- 配置迁移对照表
...
- 重构后的 `listRowConfig.tsx`（轻量级）
- 重构后的 `useCorpModuleCfg`（支持按需加载）
- 重构后的 `CompanyBase.tsx`（使用按需加载）
- 集成测试

**验收标准**：

- [ ] `listRowConfig.tsx` 改为轻量级元数据
- [ ] `useCorpModuleCfg` 支持按需加载
- [ ] `CompanyBase.tsx` 使用配置加载管理器
- [ ] 移除重复的统计字段映射
- [ ] 移除重复的过滤逻辑
- [ ] 功能与重构前保持一致
- [ ] 配置按需加载正常工作
...
- @see `src/components/company/CompanyBase.tsx`
...
| 按需加载导致闪烁     | 实现加载状态提示，优化加载体验 |
| 配置缓存策略不当     | 设计合理的缓存失效机制         |
| 回归测试不充分       | 制定详细测试用例，覆盖所有场景 |
| 性能优化效果不达预期 | 分析瓶颈，调整加载策略         |
