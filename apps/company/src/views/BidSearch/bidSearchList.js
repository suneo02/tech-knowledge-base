...
// 产品介绍页面，游客访问
class BidSearchList extends React.Component {
  constructor(props) {
    super(props)
    this.state = {
      filter: {},
  ...
  getBidSearchList = (_filter, loadingList) => {
    //执行搜索事件
    this.searchParam = {
      pageNo: this.state.pageNo,
      pageSize: this.state.pageSize,
  ...
  render() {
    ...
    return (
      <div className={`${StylePrefix}--container`}>
        <div className={`SearchList ${StylePrefix}`} id={StylePrefix} onScroll={this.scroll}>
          <SearchTitleList name="bidSearchList" jump={this.jump} />
          <div className="wrapper workspace-fix" id="SearchHome">
                ...
                <ul className={`job-ul bid-job-ul `} id="filterList" style={{ height: 'auto' }}>
                  <li className={`${StylePrefix}--filter--li--first`} style={{ position: 'relative' }}>
                    <div className={`${StylePrefix}--filter--li--first--left`}>
                      ...
                      <RadioGroup onChange={this.onChangeDate} value={this.state.oppTime}>
                      ...
                      <RangePicker onChange={this.onTimeChange} value={this.state.defaultTime} allowClear />
                    </div>
                    <a
            ...
            <div className="history-right">
              <div id="historyFocusList" className="search-r-model">
                {/* <HistoryList list={bidViewList} title={intl('271933', '最新发布')} listShowFun={this.bidViewList} /> */}
              </div>
            </div>
          </div>
        </div>
