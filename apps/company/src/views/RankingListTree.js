...
import { Breadcrumb, Input, Card, List, Tree, Empty, Row, Col, Pagination, message } from "@wind/wind-ui";
import RankingCard from '../components/RankingCard';
import { MyIcon } from '../components/Icon';
import { rankingListSearch, getRankingListType } from '../api/rankingListApi';
import * as RankingListActions from "../actions/rankingList";

import "./RankingListTree.less";

// 榜单名录列表
class RankingListTree extends React.Component {
        ...
        return (
            <div className="rankingListTree">
                <div className="pageTitle">
                    <Breadcrumb>
                        <Breadcrumb.Item onClick={() => { this.props.history.push("/rankList") }}>{this.$translation(315079)}</Breadcrumb.Item>
                        <Breadcrumb.Item>{this.$translation(315081)}</Breadcrumb.Item>
                    </Breadcrumb>
                    <Input.Search placeholder={this.$translation(261032)} value={keyword} onChange={this.inputChange} onPressEnter={() => this.treeSearch()} prefix={<MyIcon name="find" />} />
                </div>
                <div style={{ padding: "12px 16px" }}>
                    <Row gutter={15} className="container">
                        ...
                        <Col className="main_content">
                            {treeMenu && treeMenu.list && treeMenu.list.map((item,index) => (
...
const mapDispatchToProps = dispatch => {
    return {
        treeSearch: data => {
            rankingListSearch(data)
                .then(res => {
                    dispatch(RankingListActions.treeSearch({ ...res, ...data }));
                })
        },
        getRankingListType: () => {
            return getRankingListType()
                .then(res => {
