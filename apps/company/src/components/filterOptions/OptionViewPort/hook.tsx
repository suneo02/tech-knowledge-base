 ...
 */
const create_itemOption_value_label_map = (itemOption) => {
 ...
 */
export const useOptionViewportItemOption = (info: CDEFilterItem, value: string[]) => {
  // 从全局上下文中获取当前语言，用于日期格式化
  ...
  const dateFormat = (dataString) => {
    ...
    // 英文环境下直接返回原始字符串
    if (language === 'en' || window.en_access_config) {
      return dataString
    }
    return `${dataString.substring(0, 4)}年${dataString.substring(4, 6)}月${dataString.substring(6, 8)}日`
  }

  // 使用 useMemo 缓存扁平化的叶子节点列表，避免在每次渲染时都重新计算
  // 这个列表用于在处理多级复选框的自定义值时，能快速查找到 value 对应的 label
  const value_labels = useMemo(() => {
    return create_itemOption_value_label_map(info?.itemOption)
  }, [info.itemOption])
