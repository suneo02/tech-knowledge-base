// 详情字段配置
import { TableColumnDateRange } from '@/components/table/columns'
import { corpDetailListInfoDetailCfg } from '@/handle/corpModuleCfg'
import { formatCurrency } from '@/utils/common.ts'
import { wftCommon } from '@/utils/utils.tsx'
import { DateLocationO } from '@wind/icons'
import React from 'react'
import { intlNoNO as intl } from 'src/utils/intl'

export const listDetailConfig = {
  getdoublerandom: {
    rowIdx: 8,
    cmd: 'getspotcheckdetail',
    extraParams: (record, data) => {
      return {
        checkPlanNo: record.check_plan_no,
        checkTaskNo: record.check_task_no,
        companycode: data.companycode,
      }
    },
    iconNode: (_ex, _re) => {
      return (
        <span className="wi-btn-color expand-icon-getimpexp">
          {' '}
          <i></i>{' '}
        </span>
      )
    },
    notHorizontal: true,
    itemKey: [
      {
        title: intl('437410', '检查事项'),
        key: 0,
        dataIndex: 'check_item',
      },
      {
        title: intl('138492', '检查结果'),
        key: 1,
        dataIndex: 'check_result',
      },
    ],
    rowKey: ['check_item', 'check_result'],
  },
  getteleLics: {
    rowIdx: 4,
    cmd: 'detail/company/teleLicenseDetail',
    extraParams: (record, data) => {
      return {
        detailId: record.referenceId,
        companycode: data.companycode,
      }
    },
    iconNode: (_ex, _re) => {
      return (
        <span className="wi-btn-color expand-icon-getimpexp">
          {' '}
          <i></i>{' '}
        </span>
      )
    },
    itemKey: [
      intl('138677', '企业名称'),
      intl('138808', '统一社会信用代码'),
      intl('451206', '法定代表人'),
      intl('205398', '许可证号'),
      intl('411695', '企业所有制性质'),
      intl('35779', '注册资本'),
      intl('437417', '注册属地'),
      intl('35776', '注册地址'),
      intl('437398', '上市情况'),
      intl('6440', '股票代码'),
      intl('437418', '许可证业务种类'),
      intl('437401', '客户服务投诉电话'),
      intl('437399', '用户投诉量'),
      intl('437400', '用户投诉回复率'),
    ],
    rowKey: [
      'corpName',
      'creditCode',
      'legalPerson',
      'licNo',
      'corpNat',
      'regCapital|formatMoney',
      'regAreaCode',
      'regAddress',
      'listed',
      'stockCode',
      'licCat',
      'complaintTel',
      'complaintNum',
      'complaintAnswer|formatPercent',
    ],
  },
  'showLandInfo-0': {
    rowIdx: 5,
    cmd: '/detail/company/getlandannsdetail',
    extraParams: (record, param) => {
      param.detailId = record.referenceId
      param.__primaryKey = param.companycode
      return param
    },
    iconNode: (_ex, _re) => {
      return (
        <span className="wi-btn-color expand-icon-getimpexp">
          {' '}
          <i></i>{' '}
        </span>
      )
    },
    itemKey: [
      intl('437459', '宗地编号'),
      intl('205383', '行政区'),
      intl('100868', '土地用途'),
      intl('205389', '土地面积(公顷)'),
      intl('420193', '出让年限(年)'),
      intl('205386', '成交价格(万元)'),
      intl('437449', '受让单位'),
      intl('205435', '发布机关'),
      intl('437450', '公示期'),
      intl('138908', '发布日期'),
    ],
    rowKey: [
      'landNode',
      'regionCode',
      'landUsage',
      'landHa',
      'landYeasNum',
      'dealPrice|formatMoney',
      'transferee',
      'relAgency',
      'annStart|formatTime',
      'relDate|formatTime',
    ],
    itemKey1: [
      intl('437459', '宗地编号'),
      intl('205383', '行政区'),
      intl('100868', '土地用途'),
      intl('205389', '土地面积(公顷)'),
      intl('420193', '出让年限(年)'),
      intl('205386', '成交价格(万元)'),
      intl('437449', '受让单位'),
      intl('205435', '发布机关'),
      intl('437450', '公示期'),
      intl('138908', '发布日期'),
    ],
    rowKey1: [
      'landNode',
      'regionCode',
      'landUsage',
      'landHa',
      'landYeasNum',
      'dealPrice|formatMoney',
      'transferee',
      'relAgency',
      'annStart|formatTime',
      'relDate|formatTime',
    ],
    itemKey2: [
      intl('478601', '联系单位'),
      intl('478602', '单位地址'),
      intl('16758', '邮政编码'),
      intl('10057', '联系电话'),
      intl('69149', '联系人'),
      intl('478603', '电子邮件'),
    ],
    rowKey2: ['contactOrg', 'orgAddr', 'postcode', 'contactTel', 'contactPsn', 'email'],
  },
  'showLandInfo-1': {
    rowIdx: 7,
    cmd: '/detail/company/getlandpurchasedetail',
    extraParams: (record, param) => {
      param.detailId = record.referenceId
      param.__primaryKey = param.companycode
      return param
    },
    iconNode: (_ex, _re) => {
      return (
        <span className="wi-btn-color expand-icon-getimpexp">
          {' '}
          <i></i>{' '}
        </span>
      )
    },
    itemKey: [
      intl('205383', '行政区'),
      intl('437458', '电子监管号'),
      intl('205389', '土地面积(公顷)'),
      intl('437460', '土地资源'),
      intl('437451', '土地来源'),
      intl('100868', '土地用途'),
      intl('437452', '土地级别'),
      intl('437453', '供地方式'),
      intl('437474', '土地使用年限(年)'),
      intl('205386', '成交价格(万元)'),
      intl('437461', '土地使用权人'),
      intl('437475', '行业分类'),
      intl('437462', '约定容积率下限'),
      intl('437463', '约定容积率上限'),
      intl('205436', '合同签订日期'),
      intl('437476', '约定交地时间'),
      intl('437477', '约定开工时间'),
      intl('437464', '约定竣工时间'),
      intl('437465', '实际开工时间'),
      intl('437416', '实际竣工时间'),
      intl('437402', '批准单位'),
    ],
    rowKey: [
      'regionCode',
      'supervisionCode',
      'landHa',
      'landRes',
      'landOrigin',
      'landUsage',
      'levelName',
      'landProvName',
      'landYearsNum',
      'dealPrice|formatMoneyComma',
      'landUser',
      'industry',
      'volRate',
      'maxVolRate',
      'contractDate|formatTime',
      'promiseDeliverTime|formatTime',
      'promiseStartTime|formatTime',
      'promiseEndTime|formatTime',
      'realStartTime|formatTime',
      'realEndTime|formatTime',
      'approver',
    ],
  },
  'showLandInfo-2': {
    rowIdx: 7,
    cmd: '/detail/company/getlandtransdetail',
    extraParams: (record, param) => {
      param.detailId = record.referenceId
      param.__primaryKey = param.companycode
      return param
    },
    iconNode: (_ex, _re) => {
      return (
        <span className="wi-btn-color expand-icon-getimpexp">
          {' '}
          <i></i>{' '}
        </span>
      )
    },
    itemKey: [
      intl('478604', '宗地标识'),
      intl('437459', '宗地编号'),
      intl('205383', '行政区'),
      intl('205389', '土地面积(公顷)'),
      intl('100868', '土地用途'),
      intl('437452', '土地级别'),
      intl('205439', '原土地使用权人'),
      intl('205438', '现土地使用权人'),
      intl('478587', '土地使用权类型'),
      intl('437474', '土地使用年限(年)'),
      intl('478588', '土地利用状况'),
      intl('478605', '转让方式'),
      intl('205396', '转让价格(万元)'),
      intl('478589', '成交时间'),
    ],
    rowKey: [
      'parcelId',
      'parcelCode',
      'regionCode',
      'landHa',
      'landUsage',
      'landLvl',
      'oldUser',
      'curUser',
      'landPropRight',
      'landYearsNum',
      'landUseState',
      'transType',
      'transPrice|formatMoney',
      'dealTime|formatTime',
    ],
  },
  getdomainname: {
    rowIdx: 6,
    cmd: 'getdomaindetails',
    extraParams: (record, data) => {
      return {
        detailId: record.detailId,
        companycode: data.companycode,
      }
    },
    itemKey: [intl('303754', '网址名称'), intl('303755', '网址关键词'), intl('303736', '网址介绍')],
    rowKey: ['webName', 'webKeyword', 'webIntroduce'],
    iconNode: (_ex, _re) => {
      return (
        <span className="wi-btn-color expand-icon-getimpexp">
          {' '}
          <i></i>{' '}
        </span>
      )
    },
    dataCallback: (res) => {
      return res.data && res.data.length ? res.data[0] : {}
    },
  },
  'showCompanyNotice-0': {
    // 企业公示 - 股东及出资信息
    rowIdx: 4,
    cmd: 'detail/company/shareholdercontributiondetail',
    extraParams: (record, data) => {
      return {
        relatecode: record.relate_code,
        companycode: data.companycode,
      }
    },
    itemKey: [
      intl('437441', '认缴出资日期'),
      intl('437433', '认缴公示日期'),
      intl('437454', '认缴出资方式'),
      intl('232858', '认缴出资金额（万元）'),
      intl('437442', '实缴出资日期'),
      intl('437455', '实缴公示日期'),
      intl('138127', '实缴出资方式'),
      intl('437456', '实缴出资金额（万元）'),
    ],
    rowKey: [
      'promise_date|formatTime',
      'promise_notice_date|formatTime',
      'promise_method',
      'promise_amount|formatMoneyComma',
      'real_date|formatTime',
      'real_notice_date|formatTime',
      'real_method',
      'real_amount|formatMoneyComma',
    ],
    iconNode: (_ex, _re) => {
      return (
        <span className="wi-btn-color expand-icon-getimpexp">
          {' '}
          <i></i>{' '}
        </span>
      )
    },
    dataCallback: (res) => {
      return res.data && res.data.length ? res.data[0] : {}
    },
  },
  getimpexp: {
    rowIdx: 4,
    cmd: 'detail/company/impexpdetail',
    title: <span style={{ fontSize: 'small' }}>{intl('138480', '注册信息')}</span>,
    extraParams: (record, data) => {
      return {
        companyid: data.companyid,
        detailId: record.referenceId,
        companycode: data.companycode,
      }
    },
    itemKey: [
      intl('437404', '海关注册编码'),
      intl('205420', '注册海关'),
      intl('437405', '行政地区'),
      intl('437421', '经济地区'),
      intl('205421', '经营类别'),
      intl('437422', '特殊贸易区域'),
      intl('437406', '海关注销标志'),
      intl('437423', '年报情况'),
      intl('451207', '注册日期'),
      intl('437407', '报关有效期'),
      intl('437408', '行业种类'),
      intl('437424', '跨境贸易电子商务类型'),
      intl('437425', '信用等级'),
      intl('437426', '信用信息异常情况'),
    ],
    rowKey: [
      'hsCode',
      'regCust',
      'areaName',
      'ecoName',
      'bizCatName',
      'stzName',
      'customFlag',
      'annualDesc',
      'regDate|formatTime',
      'customExpires|formatTime',
      'industry',
      'bizName',
      'creditLvlName',
      'creditEx',
    ],
    iconNode: (_ex, _re) => {
      return (
        <span className="wi-btn-color expand-icon-getimpexp">
          {' '}
          <i></i>{' '}
        </span>
      )
    },
    columns: [
      null,
      null,
      null,
      null,
      null,
      null,
      null,
      null,
      null,
      {
        render: (row) => {
          if (row.customExpires == '9999/12/31' || row.customExpires == '99991231') {
            return intl('40768', '长期')
          }
          if (row.customExpires) {
            return wftCommon.formatTime(row.customExpires)
          }

          return '--'
        },
      },
    ],
  },

  'showStockMortgage-0': {
    rowIdx: 6,
    cmd: '',
    itemKey: [
      intl('138447', '出质人'),
      intl('138446', '质权人'),
      intl('354213', '出质股权标的的企业'),
      intl('205511', '质押股数(万股)'),
      intl('35935', '最新收盘价'),
      intl('478590', '疑似平仓价'),
      intl('354233', '质押日参考市值（万元）'),
      intl('354234', '最新参考市值（万元）'),
      intl('478591', '质押起始日期'),
      intl('478606', '质押截止日期'),
      intl('313177', '解押日期'),
      intl('32903', '公告日期'),
      intl('478592', '是否股权质押回购'),
      intl('313196', '是否解押'),
      [intl('354277', '质押用途')],
      [intl('478607', '相关说明')],
    ],
    rowKey: [
      'pledger_name',
      'pledgee_name',
      'plex_name',
      'amount',
      'price',
      'closedPrice',
      '_marketValue',
      'latestMarketValue',
      'startTime',
      'endTime',
      'releaseDate',
      'announceDate',
      'isBuyback',
      'isRelease',
      '_codeOfPledge',
      'brief',
    ],
    iconNode: (_ex, _re) => {
      return (
        <span className="wi-btn-color expand-icon-getimpexp">
          {' '}
          <i></i>{' '}
        </span>
      )
    },
    columns: [
      null,
      null,
      null,
      {
        render: (_row, txt) => {
          return wftCommon.formatMoney(txt, [4, ' '])
        },
      },
      {
        render: (_row, txt) => {
          return wftCommon.formatMoney(txt, [2, ' '])
        },
      },
      {
        render: (_row, txt) => {
          return wftCommon.formatMoney(txt, [4, ' '])
        },
      },
      {
        render: (_row, txt) => {
          return wftCommon.formatMoney(txt, [4, ' '])
        },
      },
      {
        render: (_row, txt) => {
          return wftCommon.formatMoney(txt, [4, ' '])
        },
      },
      null,
    ],
  },
  'showStockMortgage-1': {
    rowIdx: 6,
    cmd: '',
    itemKey: [
      intl('138447', '出质人'),
      intl('138446', '质权人'),
      intl('354213', '出质股权标的的企业'),
      intl('205511', '质押股数(万股)'),
      intl('35935', '最新收盘价'),
      intl('478590', '疑似平仓价'),
      intl('354233', '质押日参考市值（万元）'),
      intl('354234', '最新参考市值（万元）'),
      intl('478591', '质押起始日期'),
      intl('478606', '质押截止日期'),
      intl('313177', '解押日期'),
      intl('32903', '公告日期'),
      intl('478592', '是否股权质押回购'),
      intl('313196', '是否解押'),
      [intl('354277', '质押用途')],
      [intl('478607', '相关说明')],
    ],
    rowKey: [
      'pledger_name',
      'pledgee_name',
      'plex_name',
      'amount',
      'price',
      'closedPrice',
      '_marketValue',
      'latestMarketValue',
      'startTime',
      'endTime',
      'releaseDate',
      'announceDate',
      'isBuyback',
      'isRelease',
      '_codeOfPledge',
      'brief',
    ],
    iconNode: (_ex, _re) => {
      return (
        <span className="wi-btn-color expand-icon-getimpexp">
          {' '}
          <i></i>{' '}
        </span>
      )
    },
    columns: [
      null,
      null,
      null,
      {
        render: (_row, txt) => {
          return wftCommon.formatMoney(txt, [4, ' '])
        },
      },
      {
        render: (_row, txt) => {
          return wftCommon.formatMoney(txt, [2, ' '])
        },
      },
      {
        render: (_row, txt) => {
          return wftCommon.formatMoney(txt, [4, ' '])
        },
      },
      {
        render: (_row, txt) => {
          return wftCommon.formatMoney(txt, [4, ' '])
        },
      },
      {
        render: (_row, txt) => {
          return wftCommon.formatMoney(txt, [4, ' '])
        },
      },
      null,
    ],
  },
  'showStockMortgage-2': {
    rowIdx: 6,
    cmd: '',
    itemKey: [
      intl('138447', '出质人'),
      intl('138446', '质权人'),
      intl('354213', '出质股权标的的企业'),
      intl('205511', '质押股数(万股)'),
      intl('35935', '最新收盘价'),
      intl('478590', '疑似平仓价'),
      intl('354233', '质押日参考市值（万元）'),
      intl('354234', '最新参考市值（万元）'),
      intl('478591', '质押起始日期'),
      intl('478606', '质押截止日期'),
      intl('313177', '解押日期'),
      intl('32903', '公告日期'),
      intl('478592', '是否股权质押回购'),
      intl('313196', '是否解押'),
      [intl('354277', '质押用途')],
      [intl('478607', '相关说明')],
    ],
    rowKey: [
      'pledger_name',
      'pledgee_name',
      'plex_name',
      'amount',
      'price',
      'closedPrice',
      '_marketValue',
      'latestMarketValue',
      'startTime',
      'endTime',
      'releaseDate',
      'announceDate',
      'isBuyback',
      'isRelease',
      '_codeOfPledge',
      'brief',
    ],
    iconNode: (_ex, _re) => {
      return (
        <span className="wi-btn-color expand-icon-getimpexp">
          {' '}
          <i></i>{' '}
        </span>
      )
    },
    columns: [
      null,
      null,
      null,
      {
        render: (_row, txt) => {
          return wftCommon.formatMoney(txt, [4, ' '])
        },
      },
      {
        render: (_row, txt) => {
          return wftCommon.formatMoney(txt, [2, ' '])
        },
      },
      {
        render: (_row, txt) => {
          return wftCommon.formatMoney(txt, [4, ' '])
        },
      },
      {
        render: (_row, txt) => {
          return wftCommon.formatMoney(txt, [4, ' '])
        },
      },
      {
        render: (_row, txt) => {
          return wftCommon.formatMoney(txt, [4, ' '])
        },
      },
      null,
    ],
  },

  getpermission02: {
    rowIdx: 5,
    cmd: 'detail/company/getadminilicencedetail',
    itemKey: [
      intl('222773', '行政许可决定文书号'),
      intl('222813', '行政许可决定文书名称'),
      intl('138197', '证书编号'),
      intl('437447', '许可证书名称'),
      intl('222771', '许可决定日期'),
      intl('21235', '有效期'),
      intl('216395', '发证机关'),
      intl('13126', '审核类型'),
      intl('138378', '许可内容'),
    ],
    rowKey: [
      'docId',
      'docName',
      'licenceId',
      'licenceName',
      'licenceDate|formatTime',
      'startDate',
      'orgName',
      'type',
      'content',
    ],
    extraParams: (record, data) => {
      return {
        companyid: data.companyid,
        detailId: record.detailId,
        companycode: data.companycode,
      }
    },
    columns: [
      null,
      null,
      null,
      null,
      null,
      {
        render: (row) => <TableColumnDateRange startDate={row.startDate} endDate={row.endDate} />,
      },
    ],
    iconNode: (_ex, _re) => {
      return (
        <span className="wi-btn-color expand-icon-getimpexp">
          {' '}
          <i></i>{' '}
        </span>
      )
    },
  },

  'credit-0': {
    rowIdx: 4,
    cmd: 'detail/company/personCreditDetail',
    extraParams: (record, _data) => {
      return {
        detailId: record.seqId,
      }
    },
    itemKey: [
      [intl('138677', '企业名称')],
      [intl('222773', '行政许可决定文书号')],
      [intl('222813', '行政许可决定文书名称')],
      [intl('138378', '许可内容')],
    ],
    rowKey: ['companyName', 'licenseDocNo', 'licenseDocName', 'licenseContent'],
    iconNode: (_ex, _re) => {
      return (
        <span className="wi-btn-color expand-icon-getimpexp">
          {' '}
          <i></i>{' '}
        </span>
      )
    },
    dataCallback: (res) => {
      return res?.Data
    },
  },
  getfinanciallicence: {
    rowIdx: 5,
    cmd: '', // cmd为空，直接从list中读取detail进行展示
    itemKey: [
      intl('222773', '行政许可决定文书号'),
      intl('222813', '行政许可决定文书名称'),
      intl('205398', '许可证号'),
      intl('222783', '许可证名称'),
      intl('138245', '发证日期'),
      intl('148644', '批准日期'),
      intl('138143', '公示日期'),
      intl('437420', '许可机关'),
      intl('13126', '审核类型'),
      intl('138378', '许可内容'),
    ],
    rowKey: [
      'docId',
      'docName',
      'licenceId',
      'licenceName',
      'licenceDate|formatTime',
      'startDate',
      'pubDate|formatTime',
      'orgName',
      'auditType',
      'licenceContent',
    ],
    columns: [
      null,
      null,
      null,
      null,
      null,
      {
        render: (row) => {
          if (row.endDate == '9999/12/31' || row.endDate == '99991231') {
            return intl('40768', '长期')
          }
          if (row.startDate) {
            return wftCommon.formatTime(row.startDate)
          }
          return '--'
        },
      },
    ],
    iconNode: (_ex, _re) => {
      return (
        <span className="wi-btn-color expand-icon-getimpexp">
          {' '}
          <i></i>{' '}
        </span>
      )
    },
  },
  'showChattelmortgage-0': {
    rowIdx: 8,
    cmd: 'getchattlemortgagedetail',
    extraParams: (record, data) => {
      return {
        detailId: record.ob_object_id,
        companycode: data.companycode,
      }
    },
    itemKey: [
      intl('478593', '动产抵押登记编号'),
      intl('478608', '动产抵押登记日期'),
      intl('138309', '履行债务的期限'),
      intl('138391', '抵押权人'),
      intl('478614', '抵押物所有权归属'),
      intl('478609', '抵押权人证件类型'),
      intl('478615', '抵押权人证件号码'),
      intl('478616', '被担保债权数额'),
      intl('138618', '被担保债权种类'),
      intl('478617', '担保范围'),
      intl('478610', '动产抵押登记机构'),
      intl('478611', '动产抵押备注'),
      intl('478612', '抵押物描述'),
    ],
    rowKey: [
      'cm_reg_no',
      'cm_reg_date|formatTime',
      'cm_obligor_deadline|formatTime',
      'cm_mortgage_person_name',
      'cm_dpawn_owner',
      'cm_mortgage_person_card_type',
      'cm_mortgage_person_card_no',
      'cm_zq_amount|formatMoney',
      'cm_zq_type',
      'cm_guarantee_scope',
      'cm_reg_authority',
      'cm_remark',
      'dpawminfo',
    ],
    iconNode: (_ex, _re) => {
      return (
        <span className="wi-btn-color expand-icon-getimpexp">
          {' '}
          <i></i>{' '}
        </span>
      )
    },
  },
  'showChattelmortgage-1': {
    rowIdx: 8,
    cmd: 'getchattlemortgagedetail',
    extraParams: (record, data) => {
      return {
        detailId: record.ob_object_id,
        companycode: data.companycode,
      }
    },
    itemKey: [
      intl('478593', '动产抵押登记编号'),
      intl('478608', '动产抵押登记日期'),
      intl('138309', '履行债务的期限'),
      intl('138391', '抵押权人'),
      intl('478614', '抵押物所有权归属'),
      intl('478609', '抵押权人证件类型'),
      intl('478615', '抵押权人证件号码'),
      intl('478616', '被担保债权数额'),
      intl('138618', '被担保债权种类'),
      intl('478617', '担保范围'),
      intl('478610', '动产抵押登记机构'),
      intl('478611', '动产抵押备注'),
      intl('478612', '抵押物描述'),
    ],
    rowKey: [
      'cm_reg_no',
      'cm_reg_date|formatTime',
      'cm_obligor_deadline|formatTime',
      'cm_mortgage_person_name',
      'cm_dpawn_owner',
      'cm_mortgage_person_card_type',
      'cm_mortgage_person_card_no',
      'cm_zq_amount|formatMoney',
      'cm_zq_type',
      'cm_guarantee_scope',
      'cm_reg_authority',
      'cm_remark',
      'dpawminfo',
    ],
    iconNode: (_ex, _re) => {
      return (
        <span className="wi-btn-color expand-icon-getimpexp">
          {' '}
          <i></i>{' '}
        </span>
      )
    },
  },
  'historyshowChattelmortgage-0': {
    rowIdx: 8,
    cmd: 'getchattlemortgagedetail',
    extraParams: (record, data) => {
      return {
        detailId: record.ob_object_id,
        companycode: data.companycode,
      }
    },
    itemKey: [
      intl('478593', '动产抵押登记编号'),
      intl('478608', '动产抵押登记日期'),
      intl('138309', '履行债务的期限'),
      intl('138391', '抵押权人'),
      intl('478614', '抵押物所有权归属'),
      intl('478609', '抵押权人证件类型'),
      intl('478615', '抵押权人证件号码'),
      intl('478616', '被担保债权数额'),
      intl('138618', '被担保债权种类'),
      intl('478617', '担保范围'),
      intl('478610', '动产抵押登记机构'),
      intl('478611', '动产抵押备注'),
      intl('478612', '抵押物描述'),
    ],
    rowKey: [
      'cm_reg_no',
      'cm_reg_date|formatTime',
      'cm_obligor_deadline|formatTime',
      'cm_mortgage_person_name',
      'cm_dpawn_owner',
      'cm_mortgage_person_card_type',
      'cm_mortgage_person_card_no',
      'cm_zq_amount|formatMoney',
      'cm_zq_type',
      'cm_guarantee_scope',
      'cm_reg_authority',
      'cm_remark',
      'dpawminfo',
    ],
    iconNode: (_ex, _re) => {
      return (
        <span className="wi-btn-color expand-icon-getimpexp">
          {' '}
          <i></i>{' '}
        </span>
      )
    },
  },
  'historyshowChattelmortgage-1': {
    rowIdx: 8,
    cmd: 'getchattlemortgagedetail',
    extraParams: (record, data) => {
      return {
        detailId: record.ob_object_id,
        companycode: data.companycode,
      }
    },
    itemKey: [
      intl('478593', '动产抵押登记编号'),
      intl('478608', '动产抵押登记日期'),
      intl('138309', '履行债务的期限'),
      intl('138391', '抵押权人'),
      intl('478614', '抵押物所有权归属'),
      intl('478609', '抵押权人证件类型'),
      intl('478615', '抵押权人证件号码'),
      intl('478616', '被担保债权数额'),
      intl('138618', '被担保债权种类'),
      intl('478617', '担保范围'),
      intl('478610', '动产抵押登记机构'),
      intl('478611', '动产抵押备注'),
      intl('478612', '抵押物描述'),
    ],
    rowKey: [
      'cm_reg_no',
      'cm_reg_date|formatTime',
      'cm_obligor_deadline|formatTime',
      'cm_mortgage_person_name',
      'cm_dpawn_owner',
      'cm_mortgage_person_card_type',
      'cm_mortgage_person_card_no',
      'cm_zq_amount|formatMoney',
      'cm_zq_type',
      'cm_guarantee_scope',
      'cm_reg_authority',
      'cm_remark',
      'dpawminfo',
    ],
    iconNode: (_ex, _re) => {
      return (
        <span className="wi-btn-color expand-icon-getimpexp">
          {' '}
          <i></i>{' '}
        </span>
      )
    },
  },
  listInformation: corpDetailListInfoDetailCfg,
  hzpscxk: {
    rowIdx: 5,
    cmd: 'detail/company/CosmeticsProductionLicenseDetail',
    extraParams: (record, _data) => {
      return {
        detailId: record.seqId,
      }
    },
    itemKey: [
      intl('138677', '企业名称'),
      intl('437420', '许可机关'),
      intl('222783', '许可证名称'),
      intl('205398', '许可证号'),
      intl('368133', '许可证决定文书号'),
      intl('368113', '许可证决定文书名称'),
      intl('368134', '有效期开始日期'),
      intl('368135', '有效期截止日期'),
      intl('138245', '发证日期'),
      intl('138199', '证书状态'),
      intl('138378', '许可内容'),
    ],
    rowKey: [
      'companyName',
      'apprAuthority',
      'licenseName',
      'licenseNo',
      'licenseDocNo',
      'licenseDocName',
      'startDate',
      'endDate',
      'apprDate',
      'certificateStatus',
      'licenseContent',
    ],
    columns: [
      null,
      null,
      null,
      null,
      null,
      null,
      {
        render: (_, txt) => (txt ? wftCommon.formatTime(txt) : '--'),
      },
      {
        render: (_, txt) => (txt ? wftCommon.formatTime(txt) : '--'),
      },
      {
        render: (_, txt) => (txt ? wftCommon.formatTime(txt) : '--'),
      },
      null,
      null,
    ],
    iconNode: (_ex, _re) => {
      return (
        <span className="wi-btn-color expand-icon-getimpexp">
          {' '}
          <i></i>{' '}
        </span>
      )
    },
  },
  selectList: {
    rowIdx: 3,
    cmd: 'detail/company/getdirectorydetail',
    extraParams: (record, data) => {
      return {
        companycode: data.companycode,
        companyid: data.companyid,
        category: record.objectId,
      }
    },
    iconNode: (_ex, _re) => {
      return (
        <span className="wi-btn-color expand-icon-listInformation">
          {' '}
          <i></i>{' '}
        </span>
      )
    },
    notHorizontal: true,
  },
  gettaxcredit1: {
    rowIdx: 4,
    cmd: 'detail/company/gettaxcreditdetail',
    extraParams: (record, data) => {
      return {
        // companyCode: data.companycode,
        // companyName: window.__GELCOMPANYNAME__,
        // assessTime: record.assessTime,
        __primaryKey: data.companycode,
        seqIdList: record.seqIdList,
      }
    },
    itemKey: [
      {
        title: intl('138539', '纳税人识别号'),
        key: 0,
        dataIndex: 'tax_id_no',
        render: function (data, _full) {
          return wftCommon.formatCont(data)
        },
      },
      {
        title: intl('138218', '主管税务机关'),
        key: 1,
        dataIndex: 'tax_uthority',
      },
    ],
    // itemKey: [intl('138539', '纳税人识别号'), intl('478647', '税务机关')],
    rowKey: ['tax_id_no', 'tax_uthority'],
    iconNode: (_ex, _re) => {
      return (
        <span className="wi-btn-color expand-icon-listInformation">
          {' '}
          <i></i>{' '}
        </span>
      )
    },
    notHorizontal: true,
  },
  showMerge: {
    rowIdx: 7,
    cmd: '',
    itemKey: [intl('59999', '进展描述')],
    rowKey: ['tradeIntroduction'],
    iconNode: (_ex, _re) => {
      return (
        <span className="wi-btn-color expand-icon-showMerge">
          {' '}
          <i></i>{' '}
        </span>
      )
    },
  },
  enterpriseDevelopment: {
    rowIdx: 4,
    cmd: '', // cmd为空，直接从list中读取detail进行展示
    itemKey: [
      intl('32914', '公司名称'),
      intl('337893', '资质等级'),
      intl('337874', '资格状态'),
      intl('21235', '有效期'),
      intl('337875', '认定机关'),
      intl('32903', '公告日期'),
      intl('198902', '公告编号'),
      intl('337894', '公告事项'),
    ],
    rowKey: [
      'compName',
      'qualificationLevelName|formatCont',
      'qualificationStatusName|formatCont',
      'validDateStart',
      'certificationBody|formatCont',
      'declarationDate|formatTime',
      'noticeCode|formatCont',
      'noticeInfo|formatCont',
    ],
    columns: [
      null,
      null,
      {
        render: (_row, txt, _idx) => {
          if (txt) {
            return txt
          } else {
            return intl('36518', '有效')
          }
        },
      },
      {
        render: (row, _txt) => {
          // if(row.validDateEnd==("9999/12/31"||"99991231")){
          //     return intl('40768','长期')
          // }
          if (row.validDateStart || row.validDateEnd) {
            return (
              wftCommon.formatTime(row.validDateStart) + intl('271245', ' 至 ') + wftCommon.formatTime(row.validDateEnd)
            )
          }
          return '--'
        },
      },
    ],
    iconNode: (_ex, _re) => {
      return (
        <span className="wi-btn-color expand-icon-getimpexp">
          {' '}
          <i></i>{' '}
        </span>
      )
    },
  },
  // 非标违约
  getnostandard: {
    rowIdx: 6,
    iconNode: (_ex, _re) => {
      return (
        <span className="wi-btn-color expand-icon-getimpexp">
          {' '}
          <i></i>{' '}
        </span>
      )
    },
    notHorizontal: false,
    itemKey: [
      intl('478613', '金额(万)'),
      intl('20528', '发生日期'),
      [intl('478618', '事件描述')],
      [intl('478634', '信息来源')],
    ],
    rowKey: ['defaultAmount', 'defaultDate|formatTime', 'description', 'source'],
    columns: [
      {
        render: (row) => {
          return row.defaultAmount ? wftCommon.formatMoneyTemp(row.defaultAmount, [4, ' ', '', '', 4]) : '0.00'
        },
      },
      null,
      null,
      {
        render: (row) => {
          const announceId = row.announceId
          const sourceCode = row.sourceCode
          const url = row.url
          const a = ['314019000', '314023000']
          if (!url) {
            if (sourceCode === '314024000') {
              return (
                <DateLocationO
                  onClick={() => {
                    if (window.external && window.external.ClientFunc) {
                      window.external.ClientFunc(
                        JSON.stringify({
                          func: 'command',
                          isGlobal: 1,
                          cmdid: '',
                          id: announceId,
                          title: '',
                          disableuppercase: 1,
                        })
                      )
                    }
                  }}
                  onPointerEnterCapture={undefined}
                  onPointerLeaveCapture={undefined}
                  data-uc-id="HlX8JsQI7"
                  data-uc-ct="datelocationo"
                />
              )
            }
            if (a.indexOf(sourceCode) !== -1) {
              return (
                <DateLocationO
                  onClick={() => {
                    if (window.external && window.external.ClientFunc) {
                      window.external.ClientFunc(
                        JSON.stringify({
                          func: 'command',
                          isGlobal: 1,
                          cmdid: '',
                          id: announceId,
                          title: '',
                          disableuppercase: 1,
                        })
                      )
                    }
                  }}
                  onPointerEnterCapture={undefined}
                  onPointerLeaveCapture={undefined}
                  data-uc-id="5TLgS0Jvm9"
                  data-uc-ct="datelocationo"
                />
              )
            }
            if (sourceCode === '314025000') {
              return (
                <a
                  target={'_blank'}
                  href={`/wind.risk.platform/index.html#/check/lawdetail/judgement/${announceId}`}
                  rel="noreferrer"
                  data-uc-id="QAZWPHrJ_S"
                  data-uc-ct="a"
                >
                  <DateLocationO
                    onPointerEnterCapture={undefined}
                    onPointerLeaveCapture={undefined}
                    data-uc-id="KRNKnNAmDf"
                    data-uc-ct="datelocationo"
                  ></DateLocationO>
                </a>
              )
            }
          } else if (url) {
            return (
              <DateLocationO
                onClick={() => {
                  window.open(url)
                }}
                onPointerEnterCapture={undefined}
                onPointerLeaveCapture={undefined}
                data-uc-id="tLf7lUu_tf"
                data-uc-ct="datelocationo"
              />
            )
          }
          return null
        },
      },
    ],
  },
  // 担保信息：非上市发债
  showguaranteeNotMarket: {
    rowIdx: 8,
    iconNode: (_ex, _re) => {
      return (
        <span className="wi-btn-color expand-icon-getimpexp">
          {' '}
          <i></i>{' '}
        </span>
      )
    },
    notHorizontal: false,
    itemKey: [
      intl('6672', '关联关系'),
      intl('437411', '是否履行完毕'),
      intl('417694', '是否逾期'),
      intl('420166', '逾期金额(万元)'),
    ],
    rowKey: ['relatedRelationship', 'isFinishedName', 'isOutDateName', 'overdueGuaranteeAmount|formatMoneyComma'],
  },
  historyshareholder: {
    rowIdx: 5,
    cmd: 'detail/company/gethisshareholderdetail',
    extraParams: (record, _data) => {
      return {
        shareholdId: record.sharehold_id,
        shareholdName: record.sharehold_name,
      }
    },
    itemKey: [
      {
        title: intl('451201', '变更日期'),
        key: 0,
        dataIndex: 'changeDate',
        render: (_, record) => wftCommon.formatTime(record.changeDate),
      },
      {
        title: intl('417643', '变更前出资额（万元）'),
        key: 1,
        dataIndex: 'preAmount',
        render: (_, record) => formatCurrency(record.preAmount, record.preAmountUnit),
      },
      {
        title: intl('175428', '变更前持股比例'),
        key: 2,
        dataIndex: 'preShareholdingRadio',
        render: (_, record) => wftCommon.formatPercent(record.preShareholdingRadio),
      },
      {
        title: intl('437431', '变更后出资额（万元）'),
        key: 3,
        dataIndex: 'afterAmount',
        render: (_, record) => formatCurrency(record.afterAmount, record.afterAmountUnit),
      },
      {
        title: intl('437432', '变更后持股比例'),
        key: 4,
        dataIndex: 'afterShareholdingRadio',
        render: (_, record) => wftCommon.formatPercent(record.afterShareholdingRadio),
      },
    ],
    // itemKey: [intl('138539', '纳税人识别号'), intl('478647', '税务机关')],
    rowKey: ['changeDate', 'preAmount', 'preShareholdingRadio|formatPercent', 'afterAmount', 'afterShareholdingRadio'],
    iconNode: (_ex, _re) => {
      return (
        <span className="wi-btn-color expand-icon-listInformation">
          {' '}
          <i></i>{' '}
        </span>
      )
    },
    columns: [
      {
        title: intl('451201', '变更日期'),
        key: 0,
        dataIndex: 'changeDate|formatTime',
        // render: function (data, full) {
        //   return wftCommon.formatCont(data)
        // },
      },
      {
        title: intl('417643', '变更前出资额（万元）'),
        key: 1,
        dataIndex: 'preAmount',
        render: (_, record) => formatCurrency(record.preAmount, record.preAmountUnit),
      },
      {
        title: intl('175428', '变更前持股比例'),
        key: 2,
        dataIndex: 'preShareholdingRadio|formatPercent',
      },
      {
        title: intl('437431', '变更后出资额（万元）'),
        key: 3,
        dataIndex: 'afterAmount',
        render: (_, record) => formatCurrency(record.afterAmount, record.afterAmountUnit),
      },
      {
        title: intl('437432', '变更后持股比例'),
        key: 4,
        dataIndex: 'afterShareholdingRadio|formatPercent',
      },
    ],
    notHorizontal: true,
  },
}
