...
import { TableColPropsShared, TablePropsCrossShared, TablePropsVerticalShared } from '@/types'
import { safeToStringRender } from './cell/shared'
...
export const transformCrossTablePropsToVerticalTableProps = (
  props: TablePropsCrossShared,
  dataSource: any[],
  t: TIntl
): { tableProps: TablePropsVerticalShared; dataSource: any[] } => {
  ...
  const verticalColumns: TableColPropsShared[] = [
    ...
    ...columnHeadersFromData.map((header: any, index: number) => ({
      ...column,
      title: safeToStringRender(t, header, undefined),
