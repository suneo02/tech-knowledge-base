...
import { postPointBuriedWithAxios, requestToChatWithAxios } from 'gel-api'
import { isEn, t } from 'gel-util/intl'
import { FC, useState } from 'react'
import styles from './footer.module.less'
import { removeMarkdownLinks } from '@/md/handle/removeMarkdownLinks'
...
> = ({ axiosEntWeb, content, style, isBury, ...props }) => {
  ...
  return (
    <Button
      ...
      onClick={() => {
        if (isBury) {
          postPointBuriedWithAxios(axiosEntWeb, '922610370006', {
            text: content,
          })
        }
        // [追觅科技](ner:company:1196547801) 替换成 追觅科技
        copyTextAndMessage(`${removeMarkdownLinks(content)}\n\n${intlMap.copyright}`)
      }}
...
> = ({ axiosEntWeb, content, question, style, isBury, ...props }) => {
  ...
  const handleLike = () => {
    ...
    if (!liked) {
      if (isBury) {
        postPointBuriedWithAxios(axiosEntWeb, '922610370004', {
          question: question || '',
          answer: content,
          intention: '',
          feedbackType: '无',
...
> = ({ axiosChat, axiosEntWeb, content, question, questionID, style, isBury, ...props }) => {
  const [hover, setHover] = useState(false)
  ...
  const handleDislike = ({ feedbackType, detailedFeedback }: { feedbackType: string; detailedFeedback: string }) => {
    if (isBury) {
      ...
      postPointBuriedWithAxios(axiosEntWeb, '922610370005', {
        question: question || '',
        answer: content,
        intention: '',
