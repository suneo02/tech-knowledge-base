...
import { postPointBuriedWithAxios, QueryReferenceSuggest, QueryReferenceSuggestType, WindSessionHeader } from 'gel-api'
...
const generateWebUrl = (hash: string, isDev: boolean, wsid: string): string => {
  return `${isDev && 'https://wx.wind.com.cn'}${hash}&${WindSessionHeader}=${wsid}`
}
...
export const useRefLink = (
  ...
  const handleRefJump = useCallback(() => {
    ...
    postPointBuriedWithAxios(entWebAxiosInstance, '922610370015', {
      type: type as QueryReferenceSuggestType,
    })
