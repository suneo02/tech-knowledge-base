...
import { StopCircleO } from '@wind/icons'
...
const SendIcon = () => (
  ...
  >
    <g id="发送图标" stroke="none" strokeWidth="1" fill="none" fillRule="evenodd">
      <path
...
export const ChatInputSendBtn: FC<{
  className?: string
  style?: React.CSSProperties
  isLoading?: boolean
  isActive?: boolean
  onClick: () => void
}> = ({ className, style, isLoading, isActive, onClick }) => {
  const renderIcon = useMemo(() => {
    if (isLoading) {
      return (
        <StopCircleO
          className={styles['send-icon-loading']}
  ...
  return (
    <div
      className={classNames(
        styles['send-icon'],
        isActive ? styles['send-icon--active'] : '',
        isLoading ? styles['send-icon--loading'] : '',
        className
      )}
      onClick={onClick}
      style={style}
