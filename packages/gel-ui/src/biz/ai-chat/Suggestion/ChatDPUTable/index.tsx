...
import { DPUItem, DPUTableHeader } from 'gel-api'
import { ETerminalCommandId, getTerminalCommandLink } from 'gel-util/link'
import './index.less'

export const ChatDPUTable = ({ data }: { data: DPUItem }) => {
  if (!data || !data.Content || !data.Headers || !Array.isArray(data.Content) || !Array.isArray(data.Headers))
    return null
  console.log('🚀 ~ ChatDPUTable ~ data:', data)
  const columns: ColumnProps[] = data.Headers.map((header: DPUTableHeader) => ({
    title: header.Name,
    dataIndex: header.Id,
    key: header.Id,
    ellipsis: true,
