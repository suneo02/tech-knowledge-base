...
import { useXChatParser } from '@/hooks/useChat/XChatParser/base'
...
const ChatRolesTest: React.FC<ChatRolesTestProps> = ({ chatHistoryData = [] }) => {
  ...
  // 使用工厂创建消息解析器
  const parser = useXChatParser()

  /**
   * 使用智能体和解析器初始化聊天功能
   * 这提供了消息处理、状态管理和请求触发
   */
  const { parsedMessages } = useXChat<MessageRaw, MessageParsedBase>({
    agent,
    parser,
