...
const getTreeNode = (arr, level) => {
  return (
    <TreeNode title={arr[0]} key={`${level}`} icon={() => <span>123</span>}>
...
export const DETAIL_COLUMNS = {
  type0: [
    ...
    [
      {
        title: intl('451213', '省份地区'),
        dataIndex: 'purchaseUnitList',
        colSpan: 2,
        contentAlign: 'left',
        titleAlign: 'left',
        render: (text) => {
          return text && text.length && Array.isArray(text) && wftCommon.formatCont(text[0]?.region)
        },
      },
      {
        ...
        render: (text) => {
          ...
          let arr = text.split('-')
          return (
            <Tree showLine={true} selectable={false} defaultExpandAll={true}>
              {getTreeNode(arr, 0)}
            </Tree>
          )
        },
