...
function BeneficialChart({ companycode, watermask = true, saveImgName = intl('303399', '受益人图'), bottom = false }) {
  ...
  const [tabTypes, setAllTabs] = useState([])
  ...
  const getBeneficialInfo = () => {
    myWfcAjax('detail/company/getentbasicnum/' + companyCode, {
      type: 'beneficial',
    }).then(
      (data) => {
        if (data && data.ErrorCode == 0 && data.Data) {
          let tabs = []
          let beneficialOwner = Number(data.Data.beneficialOwner)
          beneficialOwner = beneficialOwner ? beneficialOwner : 0
          var beneficialNaturalPerson = Number(data.Data.beneficialNaturalPerson)
          beneficialNaturalPerson = beneficialNaturalPerson ? beneficialNaturalPerson : 0
          var beneficialInstitutions = Number(data.Data.beneficialInstitutions)
          beneficialInstitutions = beneficialInstitutions ? beneficialInstitutions : 0
          const beneficial_nums = beneficialOwner + beneficialNaturalPerson + beneficialInstitutions
          var initType = beneficialOwner ? 1 : beneficialNaturalPerson ? 2 : 3
          if (!beneficial_nums) {
            // 无数据
            setLoaded(false)
            return
          ...
          if (beneficialOwner) {
            tabs.push({
              label: intl('421585', '受益所有人'),
              val: 1,
            })
          }
          if (beneficialNaturalPerson) {
            tabs.push({
              label: intl('421600', '受益自然人'),
              val: 2,
            })
          }
          if (beneficialInstitutions) {
            tabs.push({
              label: intl('421586', '受益机构'),
              val: 3,
            })
          }
          setAllTabs(tabs)
  ...
  const getMapData = (initType) => {
    const quaryType = initType ? initType : selType
    let cmd = 'graph/company/getbeneficiaryownerroute/'
    if (quaryType == 2) {
      cmd = 'graph/company/getbeneficiarynaturalpersionroute/'
    } else if (quaryType == 3) {
      cmd = 'graph/company/getbeneficiaryorgroute/'
    }
    myWfcAjax(cmd + companyCode, {})
      .then(
        (res) => {
          if (res.ErrorCode == global.USE_FORBIDDEN) {
            // 无权限，需要弹出vip付费弹框
            VipPopup({ title: intl('303399', '受益人图') })
          } else if (res.ErrorCode == global.USE_OUT_LIMIT || res.ErrorCode == global.VIP_OUT_LIMIT) {
            // 使用超限，已在axios层处理
          } else if (res.ErrorCode == '0' && res.Data && res.Data?.relationList?.length) {
            const data = res.Data
            const maxDepth = getMaxDepth(data.nodeList)
            const endNodeTmp = []
            data.nodeList.map((t) => {
              if (!t.depth) {
                setStartNode(t)
  ...
  const saveAction = () => {
    pointBuriedByModule(922602100996)
  ...
  const refreshAction = () => {
    setLoaded(true)
    pointBuriedByModule(922602100997)
    getMapData()
  }
  const _refreshLists = (d) => {
    if (!d.linkable) return
    return linkToCompany(d.Id, d.type === 'company', d.type === 'person', linkSourceRIME)
  }

  return (
    <div className={` zzsyr-chart-instance chart-body   ${linkSourceRIME ? 'rime-zzsyr-chart-instance' : ''}`}>
      <div className="chart-tabs">
        {tabTypes.map((t) => {
          return (
            <Button
              inline
              type={selType === t.val ? 'secondary' : 'default'}
              size={'small'}
              onClick={() => {
...
function changeTheData(data) {
  ...
  data.relationList.forEach(function (nd) {
    const typeName = nd.typeName || ''
    const ratio = nd.ratio > 0 ? wftCommon.formatPercent(nd.ratio) : ''
    const beneficialType = nd.beneficialType || ''
    const routeVal = beneficialType ? beneficialType : typeName + ' ' + ratio
    var tmplink = {
      endId: nd.targetId,
      endNode: nd.targetId,
      props: routeVal,
